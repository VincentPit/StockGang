...
class TestPaperBroker:
    ...
    def test_market_buy_deducts_cash(self, broker):
        initial_cash = asyncio.get_event_loop().run_until_complete(broker.get_cash())
        order = Order("hk00700", OrderSide.BUY, OrderType.MARKET, 100, limit_price=100.0)
        asyncio.get_event_loop().run_until_complete(broker.submit_order(order))
        new_cash = asyncio.get_event_loop().run_until_complete(broker.get_cash())
    ...
    def test_market_sell(self, broker):
        ...
        asyncio.get_event_loop().run_until_complete(broker.submit_order(buy_order))
        # Then sell
        sell_order = Order("hk00700", OrderSide.SELL, OrderType.MARKET, 100, limit_price=100.0)
        asyncio.get_event_loop().run_until_complete(broker.submit_order(sell_order))
