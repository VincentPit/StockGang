...
export interface BacktestResult {
  job_id: string;
  status: JobStatus;
  pct?: number;       // 0–100 live progress
...
export interface BacktestRequest {
  ...
  trailing_stop_pct?: number;   // 0 = disabled
  take_profit_pct?: number;     // 0 = disabled
}
...
export interface ScreenRow {
  ...
  // CN-market screening metrics
  ret_20d?: number;         // 20-day return (hot-stock filter metric)
  price_52w_pct?: number;   // 52-week price position 0→1
  dist_52w_high?: number;   // fraction below 52-week peak
  vol_60d?: number;         // annualised 60-day volatility
  limit_up_60d?: number;    // 涨停 count last 60 days
  yang_ratio_60d?: number;  // fraction of 阳线 candles (阳多绿少)
}
 ...
 *
 * onUpdate is called on every tick with the full response object so callers
 * can read live pct/step fields without replacing onProgress.
 */

// ── Job listing (used to find active jobs for resume) ─────────────────────────
...
export interface TrainLoopRequest {
  symbols?: string[];
  top_n?: number;
  max_rounds?: number;   // 1–5, default 5 (self-test ladder rounds)
}
...
export interface TrainLoopResult {
  ...
  pct?: number;
  step?: string;
  found_passing?: boolean;
  rounds_run?: number;    // how many self-test rounds ran
  best_symbol?: string;
  best_config?: string;
  best_pf?: number;
  best_wr?: number;
  ...
  all_trials: TrainTrialRow[];
  error?: string;
}

export async function startTrainLoop(req: TrainLoopRequest): Promise<TrainLoopResult> {
  const res = await apiFetch(`${BASE}/train-loop`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(req),
  }, 30_000);
...
export async function getTrainLoop(jobId: string): Promise<TrainLoopResult> {
