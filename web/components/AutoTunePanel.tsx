...
import {
  startAutoTune, getAutoTune, listJobs,
...
export default function AutoTunePanel() {
  ...
  const cancelRef  = useRef(false);   // allows user to abort polling
  ...
  // ── Resume polling for an active auto-tune job on mount ──
  // Detects stale/zombie jobs: if pct doesn't change for 3 polls (~9s), stops.
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const active = jobs.find(
          (j) => j.kind === "auto_tune" && (j.status === "running" || j.status === "pending"),
        );
        if (!active) return;

        // Quick-check: is this job actually making progress?
        const peek = await getAutoTune(active.id);
        if (peek.status !== "running" && peek.status !== "pending") {
          // Already finished — just show the result
          if (peek.status === "done") setResult(peek);
          return;
        }

        setLoading(true);
        setProgress({ pct: peek.pct ?? 0, step: `Resuming — ${peek.step ?? ""}` });

        let staleTicks = 0;
        let lastPct = peek.pct ?? -1;

        const done = await new Promise<AutoTuneResult>((resolve, reject) => {
          const tick = async () => {
            if (cancelRef.current) { reject(new Error("cancelled")); return; }
            try {
              const data = await getAutoTune(active.id);
              if (cancelRef.current) { reject(new Error("cancelled")); return; }

              const curPct = data.pct ?? 0;
              setProgress({ pct: curPct, step: data.step ?? "" });

              if (data.status === "done")  { resolve(data); return; }
              if (data.status === "error") { reject(new Error(data.error ?? "Job failed")); return; }

              // Stale detection: if pct hasn't moved in 3 polls, give up
              if (curPct === lastPct) { staleTicks++; } else { staleTicks = 0; lastPct = curPct; }
              if (staleTicks >= 3) {
                reject(new Error("Job appears stale (no progress for ~9s). It may have been interrupted by a server restart. Start a new run."));
                return;
              }
    ...
    try {
      ...
      const done = await new Promise<AutoTuneResult>((resolve, reject) => {
        const tick = async () => {
          if (cancelRef.current) { reject(new Error("cancelled")); return; }
          try {
            const data = await getAutoTune(job.job_id);
            if (cancelRef.current) { reject(new Error("cancelled")); return; }
            if (data.pct != null) setProgress({ pct: data.pct, step: data.step ?? "" });
            if (data.status === "done")  { resolve(data); return; }
            if (data.status === "error") { reject(new Error(data.error ?? "Job failed")); return; }
            setTimeout(tick, 3000);
          } catch (e) { reject(e); }
      ...
      <div className="flex gap-2">
        ...
        {loading && (
          <button
            ...
            className="px-5 py-3 rounded-xl font-semibold text-sm bg-red-900/40 text-red-300 border border-red-800/50 hover:bg-red-900/60 transition-colors"
          >
            Cancel
          </button>
        )}
